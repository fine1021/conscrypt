...
final class NativeSsl {
    ...
    void initialize(String hostname, OpenSSLKey channelIdPrivateKey) throws IOException {
        ...
        if (isClient()) {
            ...
            EchParameters echParameters = parameters.getEchParameters();
            if (echParameters != null) {
                NativeCrypto.SSL_set_enable_ech_grease(ssl, this, echParameters.useEchGrease);
                if (echParameters.configList != null && !NativeCrypto.SSL_set1_ech_config_list(ssl, this, echParameters.configList)) {
